...
int main(){
   ...
   std::println("  next(100) [0, 100): {}", random.next(100));                       // half-open: [0, 100). Same as 'random(100)'

   std::println("  between [10, 20): {}", random.between(10, 20));                   // half-open
   std::println("  between [5.0f, 10.0f): {}", random.between(5.0f, 10.0f));         // half-open

   std::println("  normalized [0.0f, 1.0f): {}", random.normalized());               // float by default (normalized<double>() for double)
   std::println("  signed_norm [-1.0f, 1.0f): {}", random.signed_norm());            // float by default (signed_norm<double>() for double)

   std::println("  coin_flip(): {}", random.coin_flip());                            // fair coin
   std::println("  coin_flip(0.9f): {}", random.coin_flip(0.9f));                    // ~90% true (weighted coin)

   std::println("  bits_as<uint8_t>(): {:08b}", random.bits_as<std::uint8_t>());        // fill an 8-bit value with random bits
   std::println("  bits<24, uint32_t>(): #{:06x}", random.bits<24, std::uint32_t>());   // 24 random bits (0xRRGGBB)
   std::println("  bits<uint64_t>(55): {:055b}", random.bits<std::uint64_t>(55));       // runtime: 55 random bits in low bits
   ...
   std::println("  index(str): {}", random.index(str));                               // random index in [0, size)
   ...
   std::shuffle(vec.begin(), vec.end(), random); // Random<E> satisfies UniformRandomBitGenerator
