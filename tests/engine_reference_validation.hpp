...
namespace rnd::detail::validation {
	...
	constexpr std::array<typename Engine::result_type, N> prng_outputs(Engine rng){
	...
	constexpr bool arrays_equal(const std::array<T, N>& lhs, const std::array<U, N>& rhs){
	...
	constexpr std::uint32_t jsf32_next(jsf32_state& state) noexcept{
		const std::uint32_t e = state.a - rnd::detail::rotl(state.b, 27);
	...
	static_assert(
		arrays_equal(prng_outputs(SmallFast32{123u}), jsf32_reference_outputs()),
	...
	constexpr std::uint64_t jsf64_next(jsf64_state& state) noexcept{
		const std::uint64_t e = state.a - rnd::detail::rotl(state.b, 7);
	...
	constexpr std::uint64_t xoshiro_next(std::array<std::uint64_t, 4>& state) noexcept{
		const std::uint64_t result = rnd::detail::rotl(state[1] * 5, 7) * 9;
